...
Step 4: AI priority analysis (Claude Sonnet)
  Determines who matters, identifies clients, ranks channels

Step 5: Generate dossiers (Gemini Flash)
...
|-----|-----------|-------|-------------|
| slack-cycle | 15 min | Gemini Flash | Sync Slack + scan for TODOs + push to Honcho |
| background-sync | 1 hour | Gemini Flash | Calendar + Obsidian/Honcho dossier sync |
| linear-pr-cycle | 30 min | Gemini Flash | Check PRs + Linear tickets (if configured) |
...
~$4-5/month on Vertex AI. Gemini Flash handles 95% of calls at $0.15/M input tokens. Claude Sonnet is only used once daily for dossier merging.
...
sync-scripts/                     # Python scripts (run on cron + during setup)
  shared.py                       # Shared utilities, path constants, user config
